<script lang="ts">
	...
	onMount(async () => {
		...
		stateMachine.initalize(lander, canvas, inputManager, music, highScores);
...
{#if show}
	<div
		class="cardWrapper silkscreen-regular"
		transition:fly={{ y: 50, x: 50, duration: 1000 }}
		style="background-image: url({lander?.level?.background ??
	...
	>
		...
		<div class="paneWrapper" bind:clientWidth={canvasWidth} bind:clientHeight={canvasHeight}>
			<div in:fly={{ y: 100, duration: 1000 }} class={true ? 'renderPane' : 'hidden renderPane'}>
				<div class={lander.gameState != GameStatusEnum.Idle ? 'HUD' : 'hidden'}>
					...
					<div>
						<table class="table">
							...
							<tr
								class:badLands={lander.landerEntity?.direction > 5 &&
									lander.landerEntity?.direction < 355}
							>
								<td>ANGLE:</td>
				...
				{#if lander.gameState == GameStatusEnum.Lost}
					<div class="displayPane losepane" in:blur={{ amount: 10, duration: 1500 }}>
						...
						<button class="modeButton" on:click={() => updateHighScores(true)}>Main Menu</button>
					</div>
				{/if}
