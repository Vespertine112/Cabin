...
import { levels, type Game, Position, Vector, GameStatusEnum } from '$lib/lunarlander/gameTypes';
...
export class PlayingState implements State {
	...
	private createAndAssignParticleSystems() {
		...
		this.explodeParticleSystem = new ParticleSystem(
			this.canvas,
			this.game.landerEntity.position,
			true,
	...
	private updateParticleSystemPostitions() {
		...
		const bottomX =
			centerX -
			(this.landerEntity.height! / 2) * Math.sin((this.landerEntity.direction * Math.PI) / 180);
		const bottomY =
			centerY +
			(this.landerEntity.height! / 2) * Math.cos((this.landerEntity.direction * Math.PI) / 180);
	...
	private createAndAssignLanderEntity() {
		let lander_idle: Sprite = new Sprite(
			'/lunarlander/sprites/Lander.png',
			{ render: true },
			{
				animate: true,
				fps: 10,
		...
		let lander_thrust: Sprite = new Sprite(
			'/lunarlander/sprites/Lander_Thrust.png',
			{ render: true },
			{
				animate: true,
				fps: 15,
		...
		let lander_landed: Sprite = new Sprite(
			'/lunarlander/sprites/Lander_LittleGuys.png',
			{ render: true },
			{
				animate: true,
				fps: 15,
