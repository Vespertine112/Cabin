...
import { levels, type Game } from '$lib/lunarlander/gameTypes';
import type InputManager from '$lib/lunarlander/inputManager';
import Sprite from '$lib/lunarlander/entites/sprite';
...
export class LevelSelectState implements State {
	...
	enter(): void {
		...
		let ganymedeSprite: Sprite = new Sprite(
			'/lunarlander/sprites/Ganymede.png',
			{ render: true },
			{
				animate: true,
				fps: 17,
		...
		this.planets.push(
			{
				title: 'Pluto',
				entity: plutoEntity,
				clickCallback: () => this.clickCallbackGeneric('Pluto')
			},
			{
				title: 'Ganymede',
				entity: ganymedeEntity,
				clickCallback: () => this.clickCallbackGeneric('Ganymede')
			},
			{
				title: 'Moon',
				entity: moonEntity,
				clickCallback: () => this.clickCallbackGeneric('Moon')
			},
			{
				title: 'Mars',
				entity: marsEntity,
				clickCallback: () => this.clickCallbackGeneric('Mars')
			}
		);
	...
	update(elapsedTime: number): void {
		...
		this.planets.forEach((planet, idx) => {
			...
			planet.entity.position.x =
				this.outerPadding + this.innerPadding * idx + this.planetSize * idx;
