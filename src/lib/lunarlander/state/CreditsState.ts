...
export class CreditsState implements State {
	...
	private credits: Credit[] = [
		...
		{ source: "'Clear Skies' ", description: 'Scott Buckley' },
		{
			source: '',
			description: 'released under CC-BY 4.0. www.scottbuckley.com.au'
		}
	];
	...
	render(elapsedTime: number): void {
		...
		let largestTextWidth: number = this.credits.reduce((maxWidth, current) => {
			...
			return this.ctx.measureText(text).width > maxWidth
