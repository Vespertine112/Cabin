...
export default class InputManager {
	...
	private customGameCommands: {
	...
	}[] = [
		{
			label: 'Ship Thrust',
			keyCode: 'ArrowUp',
			command: CustomCommands.Thrust
		},
		{
			label: 'Rotate Right',
			keyCode: 'ArrowRight',
			command: CustomCommands.RotRight
		},
		{
			label: 'Rotate Left',
			keyCode: 'ArrowLeft',
			command: CustomCommands.RotLeft
		}
	];
		...
		if (this.handlers.hasOwnProperty(keyCode)) {
			if (
				(this.handlers[keyCode].options?.toggledControl &&
					!this.handlers[keyCode].options.toggledOn) ||
				!this.handlers[keyCode].options?.toggledControl
			) {
