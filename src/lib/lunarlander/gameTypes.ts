import { getRandomInt } from '$lib/lunarlander';
...
export let levels: Level[] = [
	...
	},
	{
		...
		thrustMagnitude: 2.2,
		background: '/lunarlander/backgrounds/level_four_background.png'
	}
];
...
export class Game {
	...
	update(elapsedTime: number) {
		this.oldLanderEntity = {
			position: this.landerEntity.position,
			direction: this.landerEntity.direction
		};
	...
	private checkLandingStatus(): boolean {
		...
		safeLanding =
			this.landerSpeed < 2 &&
			(this.landerEntity.direction > 355 || this.landerEntity.direction < 5);
	...
	private updateLanderPositionAndMomentum(elapsedTime: number) {
		...
		let gravityAcceleration = new Vector(
			this.gravity.x * adjustedTime,
			this.gravity.y * adjustedTime
		);
	...
	private generateTerrainAndLandingStrips() {
		...
		let landStripWidth = Math.trunc(
			(this.terrainElevationMap.length * this.level.landStripWidth) / 100
		);
